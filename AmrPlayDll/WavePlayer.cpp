...
, m_hEventDecode(NULL)
, m_ePlayStat(WavePlayer::PlayState::Play_Stop)
, m_pDecoder(NULL)
...
void WavePlayer::Play(LPCTSTR lpszFile, DWORD* pLength)
 {
	...
	@	这样就可以实现多个amr文件的混合播放
	*/
    if(m_ePlayStat != WavePlayer::PlayState::Play_Stop)
    {
        Stop();
    }
    ...
    m_ePlayStat = WavePlayer::PlayState::Play_Playing;
}
...
BOOL WavePlayer::IsPlaying() {
    return m_ePlayStat == PlayState::Play_Playing;
}